...
class cryptoService {
  ...
  Future<void> initialize() async {
    if (!_isMaked) {
      ...
      await _generateRSAkeyPair(_seed).then((v) => _my_encrypter =
          Encrypter(RSA(publicKey: _my_publicKey, privateKey: _my_privateKey)));
      await _saveServerPublicKey().then((v) =>
          _server_encrypter = Encrypter(RSA(publicKey: _server_publicKey)));
      _isMaked = true;
    } else {
  ...
  Future<void> _generateRSAkeyPair(FortunaRandom secureRandom,
  ...
  Future<void> _saveServerPublicKey() async {
    ...
    final byteData = await rootBundle.load('keyfiles/$path');
